...
public class UserQuestionService {
    ...
    public UserQuestionService(QuestionRepository questionRepository) {
    ...
    public ResponseEntity<Map<String, Object>> saveQuestion(DomainQuestion question) {
        ...
        try {
            DomainQuestion savedQuestion = questionRepository.save(question);
            response.put("mensaje", "Pregunta guardada exitosamente.");
            response.put("datos", savedQuestion);
            return new ResponseEntity<>(response, HttpStatus.CREATED);
        } catch (Exception e) {
            response.put("error", "Error interno del servidor al guardar la pregunta: " + e.getMessage());
